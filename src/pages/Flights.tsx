...
import { Plane, Calendar, Clock, Luggage, Users, ArrowRight } from 'lucide-react';
...
export default function Flights() {
  ...
  const calculatePrice = () => {
    ...
    const luggagePrice = travelClass === 'economy' && extraLuggage ? pricing.luggage : 0;
    ...
    <div className="max-w-7xl mx-auto px-4 py-16">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          ...
          <div>
            ...
            <select 
              ...
              onChange={(e) => {
                const airline = airlines.find(a => a.id === Number(e.target.value));
                if (airline) {
          ...
          <div>
            ...
            <select 
              ...
              onChange={(e) => {
                ...
                const route = selectedAirline.routes.find(r => r.from === from && r.to === to);
                if (route) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          {travelClass === 'economy' && (
            <div>
              <label className="block text-sm font-medium mb-2">Extra Luggage</label>
              <label className="flex items-center space-x-2">
                ...
                />
                <span>Add extra luggage (+{formatPrice(selectedRoute.pricing.economy.luggage)})</span>
              </label>
            </div>
          )}
          ...
          <div className="flex items-end">
            <div className="bg-gray-50 p-4 rounded-lg w-full">
              ...
              <div className="text-3xl font-bold">{formatPrice(calculatePrice())}</div>
              <div className="text-xs text-gray-500">Including taxes and fees</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          >
            Select Seats ({selectedSeats.length + returnSelectedSeats.length}/{passengers * (tripType === 'return' ? 2 : 1)})
          </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold">Select Your Seats ({travelClass})</h2>
              <button 
            ...
            <SeatMap
              selectedSeats={currentSelection === 'outbound' ? selectedSeats : returnSelectedSeats}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">{selectedAirline.name} - {travelClass.replace(/([A-Z])/g, ' $1').trim()}</p>
          </div>
          <img src={selectedAirline.logo} alt={selectedAirline.name} className="w-16 h-16 object-cover rounded-lg" />
        </div>
